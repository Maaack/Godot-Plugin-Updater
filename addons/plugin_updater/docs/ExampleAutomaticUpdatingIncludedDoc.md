...
If an update is available, then the option to update will be listed at **Project > Tools > Update Plugins... > [PLUGIN_NAME] to v...**.
...
This effectively deletes the current `addons/[PLUGIN_PATH]/` folder and replaces it with a new one. Nothing outside of `addons/` should be affected.
...
To disable automatic updates by plugin, go into the **Project > Project Settings > General** tab, and remove the entries in the `plugin_updater/plugins` setting (ex. `"res://addons/[PLUGIN_PATH]/"`). You can then close the window.
